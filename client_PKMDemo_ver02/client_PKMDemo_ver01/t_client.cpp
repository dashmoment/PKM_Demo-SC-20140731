...
pipe_client* ps = new pipe_client;
...
tracking:
		...
		//////**************CamShift***************************************
			...
			if(cam_idx == 0){
				...
				int roi_x = maxtemp.x - 10;
				int roi_y = maxtemp.y - 10;
				int roi_width = tempdata->width + 10;
				int roi_height = tempdata->height + 10;
				...
				trackWindow.x =  roi_x;
				trackWindow.y =  roi_y;
				trackWindow.width = roi_width -10;
				trackWindow.height = roi_height -10;
				cam_idx = 1;
			}
				...
				if(cam_idx == 1 && src2_hue.rows != 0 && trackWindow.width != 0){
					...
					if(cam_itr < 5){
						...
						//trackBox.center.x= trackBox.center.x - trackWindow.width/2 + 1;
						//trackBox.center.y = trackBox.center.y - trackWindow.height/2 + 1;


						circle( src2 ,final_grap , 1, Scalar(0,0,255), 1);
